  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      if (!mountedRef.current) {
        // Component unmounted while waiting for permission — clean up stream
      ...
      if (videoRef.current) {
        ...
        // ── FIX: wait for canplay event before calling play() ──
        ...
        // Only play if still mounted and stream still active
        if (mountedRef.current && streamRef.current) {
          await videoRef.current.play().catch((err) => {
            // Ignore AbortError — happens if mode switched quickly
      ...
      if (err.name === "NotAllowedError") {
        setCameraError(
          "Camera permission denied. Tap the camera icon in your browser address bar to allow access, then refresh."
        );
      } else if (err.name === "NotFoundError") {
        setCameraError("No camera found on this device. Switch to Manual mode.");
      } else if (err.name === "AbortError") {
        // Silently ignore — user switched mode before camera started
      } else {
  ...
  async function processQRData(data: string) {
    ...
    try {
      ...
      if (isBefore(now, outDate)) {
        setResult({
          success: false,
          message: `Outpass not valid yet. Valid from ${format(outDate, "dd MMM yyyy")}.`,
          outpass,
        });
        setScanning(false);
        processingRef.current = false;
      ...
      if (mode === "out") {
        ...
        setResult({
          success: true,
          message: `${outpass.studentName} scanned OUT successfully ✓`,
          outpass: { ...outpass, scannedOut: true, scannedOutAt: now_iso },
        });
        ...
        setResult({
          success: true,
          message: `${outpass.studentName} scanned IN successfully ✓`,
          outpass: { ...outpass, scannedIn: true, scannedInAt: now_iso },
        });
      ...
      <div className="flex-1 flex flex-col items-center p-4 gap-4 w-full max-w-sm mx-auto">
        ...
        <div className="flex gap-2 p-1.5 rounded-2xl w-full"
          ...
          {(["out", "in"] as ScanMode[]).map((m) => (
            <button key={m}
              ...
              style={{
                background: mode === m ? (m === "out" ? "#f59e0b" : "#22c55e") : "transparent",
        ...
        <div className="flex gap-2 w-full">
          {([
            ...
            { id: "manual" as InputMode, label: "Manual / USB", icon: Keyboard },
            ...
            <button key={id}
              ...
              style={{
                background: inputMode === id ? "rgba(92,124,250,0.12)" : "transparent",
                borderColor: inputMode === id ? "var(--brand)" : "var(--border)",
                color: inputMode === id ? "var(--brand)" : "var(--text-secondary)",
              }}>
              <Icon size={15} /> {label}
            </button>
          ))}
            ...
            <div className="relative w-full rounded-2xl overflow-hidden"
              ...
              {cameraError && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center"
                  ...
                  <button onClick={startCamera} className="btn-primary text-sm py-2 px-5">
              ...
              {scanning && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3"
                  ...
                  <p className="text-sm font-bold font-display" style={{ color: modeColor }}>
                    Processing...
                  </p>
                </div>
              )}
        ...
        {inputMode === "manual" && !result && (
          <div className="w-full">
            <div className="card text-center mb-4"
              style={{ borderColor: modeBorderColor, background: modeBg }}>
              ...
              <p className="font-bold font-display">
                {mode === "out" ? "Student Leaving Campus" : "Student Returning to Campus"}
              </p>
        ...
        {result && (
          <div className="w-full">
            <div className="card"
              ...
              {result.outpass && (
                <div className="rounded-lg p-3 flex flex-col gap-1.5 mb-4 text-sm"
                  ...
                  {[
                    ...
                    {
                      label: "Valid",
                      value: `${format(parseISO(result.outpass.outDate), "dd MMM")} – ${format(parseISO(result.outpass.inDate), "dd MMM yyyy")}`,
                    },
                  ].map(({ label, value }) => (
