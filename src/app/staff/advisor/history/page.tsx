...
import { getAllForAdvisor } from "@/lib/db";
...
export default function AdvisorHistory() {
  ...
  useEffect(() => {
    async function load() {
      if (!staff?.id) return;
      const data = await getAllForAdvisor(staff.id);
      setOutpasses(data);
      setLoading(false);
    }
    load();
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">{outpasses.length} total applications</p>
      </div>
      {outpasses.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {outpasses.map(op => (
            <OutpassReviewCard key={op.id} outpass={op} role="advisor" onAction={async () => {}} showActions={false} />
          ))}
        </div>
      )}
    </div>
